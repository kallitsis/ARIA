...
def run_impact_assessment(
    ...
    if ecoinvent_db_name not in bw.databases:
        print(f"Ecoinvent database '{ecoinvent_db_name}' not found. Please ensure the ecoinvent database is loaded.")
        return processed_df  # Return unmodified DataFrame

    # 2) Ensure 'In/out' is numeric, fill missing values
    processed_df['In/out'] = pd.to_numeric(processed_df['In/out'], errors='coerce')
    processed_df['In/out'].fillna(0, inplace=True)

    # 3) Create columns for each impact category you want to track
    #    (Add or remove columns as needed based on your lcia_methods)
    impact_cols = {
        'GWP': None,
        'ADP': None,
        'Water use': None,
    ...
    for idx, row in processed_df.iterrows():
        ...
        if results:
            ...
            # Calculate the impact for each LCIA method
            for method in lcia_methods:
                ...
                # Adjust impact by the 'In/out' value
                adjusted_impact = row['In/out'] * lca.score

                # Decide which column to store this impact in, based on the method
    ...
    # 5) Remove rows where all impact columns are NaN
