...
ARIA complements such open-source LCA software by introducing a natural language processing layer that draws drom the qualitative dimensions of goal and scope definition to rapidly quantify environmental impacts based on Brightway2. It utilises the OpenAI API to propose suitable datasets 
candidates and even suggests alternative search terms when straightforward matches cannot be found, significantly reducing the time required for inventory analysis from weeks to just hours. Moreover, ARIA’s automation extends beyond simple dataset retrieval by incorporating user-defined rules and context from the goal and scope definition—such as geographical focus or production versus waste treatment—to refine dataset selection. This qualitative dimension, where the practitioner’s context and decisions directly influence the matching process, is frequently overlooked by purely quantitative tools. Originally developed for rapid, iterative LCA in the Faraday Institution’s ReLiB project on battery recycling, ARIA now aims to make a valuable contribution to the open-source LCA software landscape by accelerating inventory analysis, enhancing reproducibility through the systematic storage of key data, and ultimately broadening the adoption of robust environmental impact assessments across industrial and academic applications

Rather than manually matching each input flow to a background dataset, ARIA automates the search and matching process.
